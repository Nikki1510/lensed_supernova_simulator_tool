...
class Supernova:
    ...
    def __init__(self, theta_E, z_lens, z_source, cosmo, source_x, source_y, bandpass):
        """
    ...
    def flux_ratio(self, model, macro_mag, micro_peak, add_microlensing):
        """
        ...
        if add_microlensing:
            zeropoint = sncosmo.ABMagSystem(name='ab').zpbandflux(self.bandpass)
            magnitude_macro = model.bandmag(self.bandpass, time=0, magsys='ab')
            magnitude_macro -= 2.5 * np.log10(macro_mag)
            magnitude_micro = magnitude_macro + micro_peak
            flux = 10**(magnitude_micro / -2.5) * zeropoint

        else:
            flux = model.bandflux(self.bandpass, time=0)
            flux *= macro_mag
    ...
    def get_app_magnitude(self, model, day, macro_mag, td_images, peak_brightness_image, micro_day, add_microlensing):
        """
        ...
        :param peak_brightness_image: array of length [num_images] containing the previously brightest observations
        :param micro_day: array of length [num_images] containing the microlensing contribution corresponding to the
                 observational time stamp
        :param add_microlensing: bool. if False: only compute macro magnification. if True: also add microlensing
                contributions to the light curves
        ...
        """
        app_mag_ps = model.bandmag(self.bandpass, time=day - td_images, magsys='ab')
...
def main():
    ...
    sn = Supernova(1.0, 0.3, 0.8, cosmo, 0.1, 0.2, 'i')
    print("SN redshift: ", sn.z_lens)
    print(sn.light_curve(1, 70))
